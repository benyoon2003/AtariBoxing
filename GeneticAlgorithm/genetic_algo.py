...
def evaluate_policy(policy, n_episodes=1, render=False, save_csv=False, model_name=None):
    policy.eval()
    ...
    if save_csv:
        with open(f"logs/{model_name}_Eval.csv", "a") as log_file:
...
def load_policy(model_name, action_size):
...
def train(pop_size, generations, elite_frac, mutation_rate, mutation_strength, episodes_per_eval, action_size, model_name):
    ...
    with open("../train_log.csv", "a") as log_file:
        log_file.write(f"{model_name},{avg_cumulative_fitness/generations}\n")
    ...
    plot_path = os.path.join("plots/", model_name + ".png")
...
def main():
    ...
    parser.add_argument("--save_csv", type=bool, default=False, help="Save CSV file containing scores after eval")
    ...
    if args.train:
        ...
        loaded_policy = load_policy(args.model_name, env.action_space.n)
        evaluate_policy(loaded_policy, n_episodes=args.episodes_per_eval, 
        render=args.render, save_csv=args.save_csv, model_name=args.model_name)
    else:
