...
class DQN():
    ...
    def update_step(self):
        ...
        with torch.no_grad():
            next_actions = self.online_model(next_states).argmax(dim=1) # use onlie model to select action
            next_q_values = self.target_model(next_states) # target model evaluates
